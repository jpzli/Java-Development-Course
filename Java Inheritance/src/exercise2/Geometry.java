...
public class Geometry{
	...
	Geometry(){ shapes = new TreeSet<Shape>(); }
	
	void add(Shape shape){ shapes.add(shape); }
	
	void showAll()
	{
		Iterator<Shape> traverse = shapes.iterator();
		while(traverse.hasNext())
			System.out.println(traverse.next());
	}
	
	void calculator(Shape shape)
	{
		DecimalFormat df = new DecimalFormat("#.00");
		System.out.print("The area, perimeter, and volume of " + shape.getName() + " is " + df.format(shape.area()) + ", " + df.format(shape.perimeter()) + ", " );
		if(shape.volume() == 0)
			System.out.print("this shape has no volume(aka 0.00).\n");
		else
			System.out.print(df.format(shape.volume()) + "\n");
	}
